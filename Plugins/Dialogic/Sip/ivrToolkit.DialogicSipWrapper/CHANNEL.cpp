...
/**
	...
	*/
	void ivrToolkit::DialogicSipWrapper::CHANNEL::open() {
		...
		printDebug("Device name = %s\n", dev_name);
		int result = gc_OpenEx(&gc_dev, dev_name, EV_ASYNC, NULL);
		printDebug("result for gc_OpenEx() = %i\n", result);
		printDebug("gc_dev = %d\n", gc_dev);
	...
	*/
	void ivrToolkit::DialogicSipWrapper::CHANNEL::printDebug(const char* format, ...) {
		char buf[1024] = "";
		va_list argptr;
		...
		BOOST_LOG_TRIVIAL(debug) << boost::format("Channel %d: %s\n") % _id % buf;
	}

	/**
	* Print information.  All channel related prints statments should go through this print.
	*/
	void ivrToolkit::DialogicSipWrapper::CHANNEL::printInfo(const char* format, ...) {
		char buf[1024] = "";
		va_list argptr;
		va_start(argptr, format);
		_vsnprintf_s(buf, 1023, format, argptr);
		...
		BOOST_LOG_TRIVIAL(info) << boost::format("Channel %d: %s\n") % _id % buf;
	}

	/**
	* Print information.  All channel related prints statments should go through this print.
	*/
	void ivrToolkit::DialogicSipWrapper::CHANNEL::printError(const char* format, ...) {
		char buf[1024] = "";
		va_list argptr;
		va_start(argptr, format);
		_vsnprintf_s(buf, 1023, format, argptr);
		...
		BOOST_LOG_TRIVIAL(error) << boost::format("Channel %d: %s\n") % _id % buf;
	}

	/**
	* Print information.  All channel related prints statments should go through this print.
	*/
	void ivrToolkit::DialogicSipWrapper::CHANNEL::printTrace(const char* format, ...) {
		char buf[1024] = "";
		va_list argptr;
		va_start(argptr, format);
		_vsnprintf_s(buf, 1023, format, argptr);
		...
		BOOST_LOG_TRIVIAL(trace) << boost::format("Channel %d: %s\n") % _id % buf;
	}
		...
		if (dx_getdig(vox_dev, &tpt, &dig, EV_SYNC) == -1) {
			...
			*/
			...
			case DG_DPD:
				...
				break;
			default:
				printError("Digit type is Unknown, %d\n", dig.dg_type[0] & 0x000f);
			}
		}
		/*
